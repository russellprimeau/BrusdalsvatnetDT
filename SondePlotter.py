# Script for making plots of hourly time series data:  
# 1. "hour_time_series" plots multivariable timeseries data in a GUI after gap checking
# 2. "hour_correlate_matrix" plots correlation between all variables (dropping record number)
...
def hour_time_series_plot(dataframe, title):
    ...
    def create_scatter_plot(variables, start_date, end_date, draw_lines=False):
        ...
        for variable in variables:
            if variable_visibility[variable].get() == 1:
                ...
                data = dataframe[(dataframe["Timestamp"] >= start_date) & (dataframe["Timestamp"] <= end_date)]
                scatter = ax.scatter(data["Timestamp"], data[variable], label=variable, s=5, alpha=0.5)
                scatter_list.append(scatter)  # Append scatter plot to the list
                if draw_lines:
                    plot_line, = ax.plot(data["Timestamp"], data[variable], marker='None',
    ...
    def insert_gaps(df, threshold):
        ...
        df_with_gaps = df_with_gaps.rename_axis('Timestamp').reset_index()
        return df_with_gaps
    ...
    variables = dataframe.columns[2:]  # Exclude 'Timestamp' and 'Record Number'
...
def hour_correlate_matrix(df, title):
